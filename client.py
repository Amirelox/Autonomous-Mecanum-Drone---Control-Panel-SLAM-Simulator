...
# Importujemy wyłącznie parametry fizycznej budowy kafelka - brak importu wymiarów N, M!
from config import (
...
class DFSController:
    def __init__(self):
        ...
        # === 🟡 NOWOŚĆ: ŚLEPA AUTONOMIA I METRYKI BOUNDING BOX ===
        self.max_logic_r = 1          # Maksymalny odkryty wiersz sieci logicznej
        self.max_logic_c = 1          # Maksymalna odkryta kolumna sieci logicznej
        self.meta_placement = "corner" # Wybór zachowania post-processingu ("corner" / "center")
        self.goal_cell = None         # Dynamicznie wyliczana lokalizacja punktu STOP (P_stop)
        self.exploration_done = False # Flaga zakończenia fazy ślepego DFS
        
        # === ⏱️ NOWOŚĆ: POMIAR CZASU ===
        self.exploration_start_time = None
        self.fast_run_start_time = None
        self.exploration_elapsed = 0.0
        self.fast_run_elapsed = 0.0
    ...
    def update_map(self, pos_x, pos_y, heading, laser_data):
        ...
        # Dynamiczne rejestrowanie i rozszerzanie wirtualnego punktu granicznego świata (Bounding Box)
        r_curr, c_curr = phys_to_logic(pos_x, pos_y)
        if r_curr % 2 == 1 and c_curr % 2 == 1:
            self.max_logic_r = max(self.max_logic_r, r_curr)
            self.max_logic_c = max(self.max_logic_c, c_curr)
    ...
    def get_command(self):
        ...
        # Ograniczenie hamowania wyzwalane dynamicznie obliczonym punktem STOP
        if self.fast_run and self.target_logic == self.goal_cell and self.goal_cell is not None:
    ...
    def reset_logic_and_maps(self):
        """Kompleksowe czyszczenie struktur dla nowej sesji labiryntu."""
            ...
            async for msg in ws:
                ...
                if "maze_id" in data:
                    if data["maze_id"] < controller.current_maze_id:
                        ...
                        controller.reset_logic_and_maps()
                ...
                if controller.teleporting:
                    if not is_at_meta:
                        ...
                        print("⚡ Teleportacja udana! Robot ustawiony na bloku startowym (1, 1). Rozpoczynam FAST RUN!")
                    else:
                        continue
                ...
                # ============================================================
                # MASZYNA STANÓW: DETEKCJA KOŃCA ŚLEPEJ EKSPLORACJI DFS
                # ============================================================
                if not controller.finished and not controller.fast_run and not controller.exploration_done:
                    # Kryterium stopu: Pusty stos, powrót na (1,1) i zbadanie czegoś więcej niż punktu wyjścia
                    if len(controller.path_stack) == 0 and controller.current_logic_pos == (1, 1) and len(controller.visited) > 1 and controller.target_logic is None:
                        controller.exploration_done = True
                        
                        # ⏱️ Zapisz czas eksploracji
                        if controller.exploration_start_time is not None:
                            controller.exploration_elapsed = time.time() - controller.exploration_start_time
                            print(f"⏱️ Czas eksploracji: {controller.exploration_elapsed:.2f}s")
                        
                        # 2. Matematyczny Post-processing (Dedykacja parametrów jaskini)
                        discovered_n = (controller.max_logic_r + 1) // 2
                        discovered_m = (controller.max_logic_c + 1) // 2
                        print(f"🟡 [ANALIZA LOGICZNA] Wykryte gabaryty świata: N={discovered_n}, M={discovered_m}")
                        
                        if controller.meta_placement == "corner":
                            controller.goal_cell = (controller.max_logic_r, controller.max_logic_c)
                        else:
                            # Tryb środek: Row/Col wyliczone z dzielenia całkowitego
                            g_r = 2 * (discovered_n // 2) + 1
                            g_c = 2 * (discovered_m // 2) + 1
                            controller.goal_cell = (g_r, g_c)
                        
                        print(f"📍 [ZAMROŻENIE CELU] Dynamicznie wyznaczona lokalizacja punktu STOP (P_stop): {controller.goal_cell}")
                        ...
                        # ⏱️ Start pomiaru czasu fast run
                        controller.fast_run_start_time = time.time()
                        
                        await ws.send(json.dumps({"cmd": "teleport_to_start"}))
                        print("🏁 EXPLORATION COMPLETE! Najkrótsza droga BFS wyznaczona. Teleportacja na start...")
                        continue
                
                # Zakończenie fazy wyścigu (Fast Run) po osiągnięciu celu
                if is_at_meta and controller.fast_run and not controller.finished and not controller.teleporting:
                    # ⏱️ Zapisz czas fast run
                    if controller.fast_run_start_time is not None:
                        controller.fast_run_elapsed = time.time() - controller.fast_run_start_time
                        print(f"⏱️ Czas Fast Run: {controller.fast_run_elapsed:.2f}s")
                    ...
                    print("🏆 FAST RUN ZAKOŃCZONY SUKCESEM! FINISHED!")
                    continue
                ...
                try:
                    ...
                    if cmd_from_dashboard == "reset":
                        ...
                        # Przechwytywanie zmiany trybu lokalizacji mety z poziomu HMI
                        if "meta_placement" in cmd_from_dashboard:
                            controller.meta_placement = cmd_from_dashboard["meta_placement"]
                            command_queue.task_done()
                            continue
                except queue.Empty:
                    pass
